...
sleep 15
[ "$HOSTNAME" == "ip-172-31-44-141" ] && aws dynamodb create-table --table-name g1q1 --attribute-definitions AttributeName=airport,AttributeType=S --key-schema AttributeName=airport,KeyType=HASH --billing-mode PAY_PER_REQUEST >/dev/null 2>&1
[ "$HOSTNAME" == "ip-172-31-44-141" ] && aws dynamodb create-table --table-name g1q2 --attribute-definitions AttributeName=airline,AttributeType=S --key-schema AttributeName=airline,KeyType=HASH --billing-mode PAY_PER_REQUEST >/dev/null 2>&1
[ "$HOSTNAME" == "ip-172-31-44-141" ] && aws dynamodb create-table --table-name g1q3 --attribute-definitions AttributeName=dayofweek,AttributeType=S --key-schema AttributeName=dayofweek,KeyType=HASH --billing-mode PAY_PER_REQUEST >/dev/null 2>&1
