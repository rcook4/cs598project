...
FROM
(
      SELECT origin, dest, uniquecarrier, DENSE_RANK() OVER (PARTITION BY origin, dest ORDER BY fifteen_minutes_late_percentage ASC) as arrival_performance_rank, fifteen_minutes_late_percentage
      FROM
      (
