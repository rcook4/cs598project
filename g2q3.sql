...
FROM
(
      SELECT origin, dest, uniquecarrier, RANK() OVER (PARTITION BY origin, dest ORDER BY fifteen_minutes_late_percentage, uniquecarrier) as arrival_performance_rank, fifteen_minutes_late_percentage
      FROM
      (
